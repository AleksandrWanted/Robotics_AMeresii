...
func MyHeatGetDeviceInfo(login, key string, deviceID int) (*Data, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		case 0:
			return res.Data, nil
		default:
			return nil, fmt.Errorf("getting device info filed: error code %d", res.Err)
		}
	}
}

func MyHeatSetEnvGoal(reqParams *MyHeatRequestBody) (*Data, error) {
	timeout := os.Getenv("DEFAULT_REQUEST_TIMEOUT")
	_timeout, err := strconv.Atoi(timeout)
	if err != nil {
		return nil, fmt.Errorf("invalid timeout for request: %v", err)
	...
	body := MyHeatRequestBody{
		...
		ObjID:      reqParams.ObjID,
		Goal:       reqParams.Goal,
		ChangeMode: reqParams.ChangeMode,
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		case 0:
			return res.Data, nil
		default:
			return nil, fmt.Errorf("set object goal value filed: error code %d", res.Err)
		}
	}
}
